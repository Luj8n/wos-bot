...
fn get_env_var(name: &str) -> String {
  dotenv::var(name).unwrap_or_else(|_| panic!("Couldn't find {} variable in the .env file", name))
}
...
fn get_possible_words(
  ...
  println!("Time taken to process the word list: {:?}", diff);

  let letters = word.to_lowercase().bytes().counts();

  let start_time = Instant::now();

  let mut selected: Vec<_> = word_list
    .filter(|w| {
  ...
  selected.par_sort();
  selected.par_sort_by(|a, b| Ord::cmp(&a.len(), &b.len()));

  let diff = start_time.elapsed().as_millis();
  println!("Time taken to find the words: {:?}\n", diff);
